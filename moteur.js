...
// Heure de passage (2026-09-10) : le soleil tourne pendant la marche. Chaque
// segment est lu au pas de 15 min ou le marcheur y passe, estime par sa
// distance au depart divisee par cette vitesse (MIROIR de graph.HeureDePassage).
export const VITESSE_MARCHE_M_S = 1.3;
...
export class MoteurOmbra {
  ...
  async chargeOmbres(date) {
    // Incremental : les ombres d'une date sont conservees par tuile, et un
    // appel ulterieur ne charge que les tuiles arrivees depuis (chargement a
    // la demande de route()). Avant le 2026-09-07, seules les tuiles presentes
    // au PREMIER appel recevaient leurs ombres : a l'echelle de Paris, tout
    // trajet sortant du quartier de depart perdait ses ombres. Le contenu de
    // this.ombres.get(date) et la lecture dans _fractionSoleil sont inchanges.
    const pasListe = this.manifest.dates[date];
    if (!pasListe) throw new Error(`date non exportee : ${date}`);
    if (!this.ombres.has(date)) this.ombres.set(date, { pas: pasListe, v: new Map(), conf: new Map(), tuiles: new Set() });
    const o = this.ombres.get(date);
    for (const id of this.tuilesChargees) {
      if (o.tuiles.has(id)) continue;
      const t = await this.chargeurOmbres(date, id);
      o.tuiles.add(id);
      const brut = Uint8Array.from(atob(t.v), (c) => c.charCodeAt(0));
      const confPack = Uint8Array.from(atob(t.conf), (c) => c.charCodeAt(0));
      const nPas = t.pas.length;
      t.seg.forEach((i, p) => {
        o.v.set(i, brut.subarray(p * nPas, (p + 1) * nPas));
  ...
  /** Pas de passage de chaque arete (MIROIR de graph.HeureDePassage.indices_pas) :
   * distance a vol d'oiseau du depart (projete sur son trottoir) au MILIEU de
   * l'arete (moyenne de ses deux noeuds), divisee par VITESSE_MARCHE_M_S,
   * arrondie au pas inferieur et bornee au dernier pas exporte. Meme ordre
   * d'operations que Python. Les aretes sans ombre (traversees, attaches)
   * gardent le pas de depart : leur cout ne depend pas de l'heure. Calcul
   * paresseux : seules les aretes que l'A* explore sont evaluees.
   * Une seconde passe le long du trace a ete mesuree et ecartee le
   * 2026-09-10 (bench/heure/RESULTATS.md) : pas de gain net, une perte de 3,5
   * points sur un trajet de 20 minutes. */
  _pasDePassage(indexPasDepart, nPas, departXY) {
    const pasSecondes = this.manifest.pas_minutes * 60;
    const memo = new Map();
    return (arete) => {
      if (arete.type !== "trottoir") return indexPasDepart;
      let idx = memo.get(arete.i);
      if (idx === undefined) {
        const s = this.segments.get(arete.i);
        const a = this.noeuds.get(s.a), b = this.noeuds.get(s.b);
        const d = Math.hypot((a[0] + b[0]) / 2 - departXY[0], (a[1] + b[1]) / 2 - departXY[1]);
        idx = Math.min(indexPasDepart + Math.floor(d / VITESSE_MARCHE_M_S / pasSecondes), nPas - 1);
        memo.set(arete.i, idx);
      }
      return idx;
    };
  }

  /** pasDe : arete -> index du pas dont on lit l'ombre (fixe, ou heure de passage). */
  _astar(attDepart, attArrivee, date, pasDe, k, facteurTraversee) {
    ...
    while (tas.taille) {
      ...
      for (const { vers, arete } of voisins) {
        ...
        const g = gScore.get(n) + coutArete(arete, k, this._fractionSoleil(date, pasDe(arete), arete), facteurTraversee);
    ...
    const aretes = [];
    let n = ARRIVEE;
    // Chaque arete porte le pas dont son ombre a ete lue (`pas`) : c'est
    // celui que detaillerTrace et le guidage doivent relire.
    while (n !== DEPART) { const [prec, arete] = precedent.get(n); aretes.push({ de: prec, vers: n, arete, pas: pasDe(arete) }); n = prec; }
    aretes.reverse();
    let distance = 0, cout = 0, ombre = 0, soleil = 0, nonEvaluee = 0, traversees = 0;
    for (const { arete, pas } of aretes) {
      const fs = this._fractionSoleil(date, pas, arete);
      distance += arete.l;
   ...
   *
   * `heurePassage` (vrai par defaut depuis le 2026-09-10) : chaque segment est
   * lu au pas ou le marcheur y passe, estime a vol d'oiseau depuis le depart
   * (cf. _pasDePassage). Le cout d'une arete reste fixe avant la recherche :
   * A* et le plafond sont inchanges.
   * `heurePassage: false` relit tout a l'heure de depart (comportement
   * d'avant, conserve pour les fixtures de parite qui le testent). */
  async route({ depart, arrivee, k = 0, date, heure, facteurTraversee = 1.0, plafondDetour = PLAFOND_DETOUR_DEFAUT, chargerToutes = false, heurePassage = true }) {
    ...
    const avecPlafond = (pasDe) => {
      const base = this._astar(attDepart, attArrivee, date, pasDe, 0, facteurTraversee);
      ...
      if (k > 0) {
        kEffectif = k;
        resultat = this._astar(attDepart, attArrivee, date, pasDe, kEffectif, facteurTraversee);
    ...
    if (!heurePassage) return avecPlafond(() => indexPas);
    const pasDe = this._pasDePassage(indexPas, pasListe.length, attDepart.point_projete);
    const resultat = avecPlafond(pasDe);
    if (!resultat.reachable) return resultat;
    // `_pasDe` : la regle de lecture, pour que le test de parite puisse
    // rejouer un chemin de reference sur les memes donnees.
    return { ...resultat, heure_passage: true, _pasDe: pasDe };
  }
  ...
  detaillerTrace(resultat, departWgs, arriveeWgs) {
    ...
    for (const { de, vers, arete, pas } of resultat.aretes) {
      if (arete.type === "trottoir") {
        const s = this.segments.get(arete.i);
        // Pas de passage de l'arete (heure de passage), sinon celui du depart.
        const ip = pas ?? indexPas;
        const val = o.v.get(arete.i)[ip];
        const faible = o.conf.get(arete.i)[ip] === 1;
        const indet = val === 255;
