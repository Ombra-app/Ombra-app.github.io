  ...
  demarrer(surPosition, surErreur) {
    ...
    this.id = navigator.geolocation.watchPosition(
      (p) => surPosition({ lon: p.coords.longitude, lat: p.coords.latitude,
                           ...
                           // cap et vitesse GPS (null quand l'appareil ne les donne pas ou a l'arret)
                           cap: typeof p.coords.heading === "number" && !Number.isNaN(p.coords.heading) ? p.coords.heading : null,
                           vitesse: typeof p.coords.speed === "number" && !Number.isNaN(p.coords.speed) ? p.coords.speed : null }),
      (e) => surErreur?.(e),
...
export class SourceSimulee {
  ...
  positions() {
    ...
    for (let k = 0, parcouru = 0; parcouru <= total + 1e-9; k++, parcouru = k * pasM) {
      ...
      // Cap et vitesse "GPS" : direction du deplacement vrai depuis le point precedent.
      let cap = null;
      if (precedent) {
        const dx = (v.lon - precedent.lon) / degParMetreLon(v.lat), dy = (v.lat - precedent.lat) / degParMetreLat;
        if (Math.hypot(dx, dy) > 0.2) cap = ((Math.atan2(dx, dy) * 180) / Math.PI + 360) % 360;
      }
      precedent = v;
      ...
      sortie.push({
        ...
        t: k * this.pas_ms,
        cap, vitesse: cap === null ? 0 : this.vitesse_ms,
  ...
  demarrer(surPosition) {
    ...
    this.minuteur = setInterval(() => {
      ...
      const p = suite[i++];
      surPosition({ lon: p.lon, lat: p.lat, precision_m: p.precision_m, t: p.t, cap: p.cap, vitesse: p.vitesse });
    }, cadence);
  }
